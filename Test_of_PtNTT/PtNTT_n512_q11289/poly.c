...
void poly_mul_pointwise(poly *r, const poly *a, const poly *b)
{
  ...
  {
    t            = montgomery_reduce(3186*b->coeffs[i]); /* t is now in Montgomery domain 3186->12289, 4613->7681*/
...
void poly_half_mul_pointwise(poly_half *r, const poly_half *a, const poly_half *b)
{
  ...
  {
    t            = montgomery_reduce_256(4613*b->coeffs[i]); /* t is now in Montgomery domain */
    r->coeffs[i] = montgomery_reduce_256(a->coeffs[i] * t);  /* r->coeffs[i] is back in normal domain */
  }
}
...
void poly_half_add(poly_half *r, const poly_half *a, const poly_half *b)
{
  int i;
  for(i=0;i<NEWHOPE_N/2;i++)
    r->coeffs[i] = (a->coeffs[i] + b->coeffs[i]) % 7681;
}
...
void poly_ntt_256(poly_half *r)
{
  mul_coefficients_256(r->coeffs, psis_bitrev_montgomery_256);
  ntt_256((uint16_t *)r->coeffs, omegas_bitrev_montgomery_256);
}
...
void poly_invntt_256(poly_half *r)
{
  bitrev_vector_256(r->coeffs);
  ntt_256((uint16_t *)r->coeffs, omegas_inv_bitrev_montgomery_256);
  mul_coefficients_256(r->coeffs, psis_inv_montgomery_256);
  for(int i=0;i<256;i++)
  {
    if(r->coeffs[i]>=7681)
      r->coeffs[i]=r->coeffs[i]%7681;
  }
}
/*************************************************
* Name:        poly_invntt
