...
extern const uint16_t omegas_inv_bitrev_montgomery_128[];
extern const uint16_t psis_inv_montgomery_128[];
extern const uint16_t zetas_128[];
...
void ntt_128(uint16_t *p) 
{
  ...
  {
    for(start = 0; start < KYBER_N/2; start = j + (1<<level)) 
    {
      zeta = zetas_128[k++];
      for(j = start; j < start + (1<<level); ++j) 
      {
...
void invntt_128(uint16_t * a)
{
  ...
  for(level=0;level<7;level++)
  {
    ...
    {
      jTwiddle = 0;
      for(j=start;j<KYBER_N/2-1;j+=2*(1<<level))
      {
        W = omegas_inv_bitrev_montgomery_128[jTwiddle++];
        temp = a[j];
  ...
  for(j = 0; j < KYBER_N/2; j++)
    a[j] = montgomery_reduce((a[j] * psis_inv_montgomery_128[j]))%KYBER_Q;
}
