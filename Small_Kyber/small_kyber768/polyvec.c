...
void polyvec_compress_ciphertext(unsigned char *r, const polyvec *a)
{
  int i,j,k;
  uint16_t t[4];
  for(i=0;i<KYBER_K;i++)
...
void polyvec_decompress_ciphertext(polyvec *r, const unsigned char *a)
{
  int i,j;
  for(i=0;i<KYBER_K;i++)
  {
...
//   for(j=0;j<KYBER_N;j++)
//   {
//     t = montgomery_reduce(1674* (uint32_t)b->vec[0].coeffs[j]); // 1674 = 2^{2*18} % q
//     r->coeffs[j] = montgomery_reduce(a->vec[0].coeffs[j] * t);
//     for(i=1;i<KYBER_K;i++)
//     {
//       t = montgomery_reduce(1674* (uint32_t)b->vec[i].coeffs[j]);
...
int polyvec_pt_ntt3(polyvec *s, poly_halfvec *s0, poly_halfvec *s1, poly_halfvec *s2)
{
  ...
  for(i=0;i<KYBER_K;i++)
  {
    poly_pt_ntt3(&s->vec[i], &s0->vec[i], &s1->vec[i], &s2->vec[i]);
  }
}
...
int polyvec_pt_ntt_bowtiemultiply(poly *p, polyvec *a, poly_halfvec *s0, poly_halfvec *s1, poly_halfvec *s2)
{
  ...
  poly temp;
  //set value to p;
  poly_pt_ntt_bowtiemultiply(p, &a->vec[0], &s0->vec[0], &s1->vec[0], &s2->vec[0]);

  for(i=1;i<KYBER_K;i++)
  {
    poly_pt_ntt_bowtiemultiply(&temp, &a->vec[i], &s0->vec[i], &s1->vec[i], &s2->vec[i]);
    poly_add(p, p, &temp);
  } 
  
  // for(j=0;j<KYBER_N;j++) 
  // {
  //   p->coeffs[j] = barrett_reduce(p->coeffs[j]);
  // }
