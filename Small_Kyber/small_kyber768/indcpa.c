...
static void unpack_ciphertext(polyvec *b, poly *v, const unsigned char *c)
{
  ...
  poly_decompress(v, c+KYBER_K*320);
}
                   ...
                   unsigned char *sk)
{
  ...
  poly_halfvec vec_s0, vec_s1, vec_s2;
  ...
  //polyvec_ntt(&skpv);
  polyvec_pt_ntt3(&skpv, &vec_s0, &vec_s1, &vec_s2);
  ...
  for(i=0;i<KYBER_K;i++)
  {
    polyvec_pt_ntt_bowtiemultiply(&pkpv.vec[i], a+i, &vec_s0, &vec_s1, &vec_s2);
  }

  // polyvec_ntt(&pkpv);
  polyvec_inv_ptntt(&pkpv);
               ...
               const unsigned char *coins)
{
  ...
  poly_halfvec vec_sp0, vec_sp1, vec_sp2;
  poly v, k, epp;
 ...
 polyvec_pt_ntt2(&pkpv);
  ...
  // polyvec_ntt(&sp);
  polyvec_pt_ntt3(&sp, &vec_sp0, &vec_sp1, &vec_sp2);
  ...
  for(i=0;i<KYBER_K;i++)
  {
    polyvec_pt_ntt_bowtiemultiply(&bp.vec[i], at+i, &vec_sp0, &vec_sp1, &vec_sp2);
  }
  ...
  polyvec_inv_ptntt(&bp);
  
  polyvec_add(&bp, &bp, &ep);
 
  //polyvec_pointwise_acc(&v, &pkpv, &sp);
  polyvec_pt_ntt_bowtiemultiply(&v, &pkpv, &vec_sp0, &vec_sp1, &vec_sp2);
               ...
               const unsigned char *sk)
{
  ...
  polyvec_pt_ntt_bowtiemultiply(&mp, &bp, &vec_s0, &vec_s1, &vec_s2);
