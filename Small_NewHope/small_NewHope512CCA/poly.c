...
void poly_uniform(poly *a, const unsigned char *seed)
{
  ...
  for(i=0;i<NEWHOPE_N/64;i++) /* generate a in blocks of 64 coefficients */
  {
    ...
    while(ctr < 64) /* Very unlikely to run more than once */
    {
      ...
      {
        val = (buf[j] | ((uint16_t) buf[j+1] << 8))& 0x1fff;
        if(val < NEWHOPE_Q)
        {
...
void poly_quarter_mul_pointwise(poly_quarter *r, const poly_quarter *a, const poly_quarter *b)
{
  ...
  {
    t            = montgomery_reduce(4613*b->coeffs[i]); /* t is now in Montgomery domain */
...
void poly_sub(poly *r, const poly *a, const poly *b)
{
  int i;
  for(i=0;i<NEWHOPE_N;i++)
    r->coeffs[i] = (a->coeffs[i] + 4*NEWHOPE_Q - b->coeffs[i]) % NEWHOPE_Q;
}
/*************************************************
* Name:        poly_ntt
* 
...
**************************************************/
void poly_ntt_128(poly_quarter *r)
{
  mul_coefficients_128(r->coeffs, psis_bitrev_montgomery_128);
  ntt_128((uint16_t *)r->coeffs, omegas_bitrev_montgomery_128);
}
...
* Arguments:   - poly *r: pointer to in/output polynomial
**************************************************/
void poly_invntt_128(poly_quarter *r)
{
  bitrev_vector_128(r->coeffs);
  ntt_128((uint16_t *)r->coeffs, omegas_inv_bitrev_montgomery_128);
  mul_coefficients_128(r->coeffs, psis_inv_montgomery_128);
  for(int i=0;i<128;i++)
  {
...
* Name:        positive_mod
* 
* Description: Mod the number into [0,q]
*              Input is a number with 32 bits
*              Output is the equal to the input in the case of mod q
*
* Arguments:   - int32_t r: the number with 32 bits as in/output
**************************************************/
...
/*************************************************
* Name:        normal_poly_mul
* 
* Description: Compute the multiplication of two polynomials with N coefficients
